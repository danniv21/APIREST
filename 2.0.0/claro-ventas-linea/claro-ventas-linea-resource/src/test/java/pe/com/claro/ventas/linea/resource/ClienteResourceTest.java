...
public class ClienteResourceTest
{
    ...
    public void getClienteAll() throws Exception {
        ...
        cli.setDireccions(lista);
        ListCliente.add(cli);
        when(customerService.getAllClientesPaginated(filerBean.getStart(), filerBean.getSize())).thenReturn(ListCliente);
        Response response = clienteResource.getClienteAll(filerBean);
        List<Cliente> value = mapper.readValue(response.getEntity().toString(),  mapper.getTypeFactory().constructCollectionType(List.class, Cliente.class));
